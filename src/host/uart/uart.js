 ...
 * 
 */
class Serial @ "xs_uart_destructor" {
	constructor(dictionary) @ "xs_uart";

	/* set read or write timeout in ms */
	setTimeout(timeout) @ "xs_uart_setTimeout";

	/* set baudrate (to change after intial configuration) */
	setBaudrate(baud) @ "xs_uart_setBaudrate";

	/* is data available to read */
	available() @ "xs_uart_available";
	...
	readBytes(params) @ "xs_uart_readBytes";
	...
	readBytesUntil(params) @ "xs_uart_readBytesUntil";
