...
void xs_uart_read(xsMachine *the) {
	modSerial serial = xsmcGetHostData(xsThis);
	int ret;
	uint8_t val;
 ...
 */
void xs_uart_readBytes(xsMachine *the) {
	modSerial serial = xsmcGetHostData(xsThis);
	int len = 0;
	int ret;
	char *str;
...
void xs_uart_readBytesUntil(xsMachine *the) {
	modSerial serial = xsmcGetHostData(xsThis);
	char *terminators;
	int len;
	char *str;
...
void xs_uart_destructor(void *data) {
