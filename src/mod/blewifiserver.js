...
export default class WiFiServer extends BLEServer {
  ...
  constructor(deviceName, callback) {
    ...
    if (callback) {
      this.connectToWiFiNetwork = callback;
    }
  }
  ...
  onCharacteristicNotifyDisabled(characteristic) {
    if ("status" == characteristic.name) delete this.notify;
  }
  onCharacteristicNotifyEnabled(characteristic) {
    if ("status" == characteristic.name) this.notify = characteristic;
  }
  ...
  connectToWiFiNetwork(ssid, password) {
    let dictionary = { ssid };
    if (password) dictionary.password = password;

    this.#monitor?.close();
    this.#monitor = new WiFi(dictionary, (msg) => {
      switch (msg) {
        case WiFi.connected:
          this.doStatusNotification(
            `Wi-Fi connected. Waiting for IP addreess...`
          );
          break;
        case WiFi.gotIP:
  ...
  doStatusNotification(message) {
    trace(message, "\n");
    if (this.notify) this.notifyValue(this.notify, message);
  }
}
